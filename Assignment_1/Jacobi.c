...
double* solve_Jacobi(double **A, double *B, int n){
    ...
    int i;
    int stop_iter = 150;
    double error_margin = 1e-6, error = 0;
    ...
    do
    {
        ...
        for(row_op = 0; row_op < n; row_op++)
        {
            ...
            for(column_op = 0; column_op < n; column_op++){
                if(column_op != row_op) row_sum += A[row_op][column_op] * x_k[column_op];
            }

            x_k1[row_op] = (B[row_op] - row_sum)/A[row_op][row_op];
        }
    ...
    free(x_k);
    printf("Number of Iterations for Jacobi - %d", no_iter);
    return x_k1;
}